...
function onConnect() {
  ...
  var message = new Paho.Message("Hello to unity, from JS");
  message.destinationName = "M2MQTT_Unity/test";
  client.send(message);
...
function sendMessage(messageStr) {
  ...
  client.subscribe("M2MQTT_Unity/test");
  var message = new Paho.Message(messageStr);
  message.destinationName = "M2MQTT_Unity/test";
  client.send(message);
...
client.on('connect', function () {
  ...
  client.subscribe('M2MQTT_Unity/test', function (err) {
    if (!err) {
      // Publish a message to a topic
      console.log('hello, no errors in subscribe!')
...
// another thing to try:   ws://localhost:8080/
...
export function plotPredictions(
  ...
  if (topK != null) {
    ...
    sendMessage(topWord + " " + wordsAndProbs[0][1].toFixed(6)); 
    //client.publish("M2MQTT_Unity/test", topWord + " " + wordsAndProbs[0][1].toFixed(6));
    console.log(
